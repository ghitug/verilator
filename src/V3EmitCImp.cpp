...
class EmitCImp final : public EmitCFunc {
    ...
    bool m_needsFullSyms = true;  // Current output file needs the complete symbol table
    ...
    // METHODS
    // Whether the common implementation file of 'modp' needs the complete symbol table.
    // Only variable initializers are emitted from the AST here (see doCommonImp); the rest is
    // generated by this file and refers to the symbol table state base class only. Functions
    // are emitted by emitCFuncImp, into files of their own.
    static bool commonImpNeedsFullSyms(const AstNodeModule* modp) {
        if (!modp) return false;
        for (AstNode* nodep = modp->stmtsp(); nodep; nodep = nodep->nextp()) {
            if (!VN_IS(nodep, Var)) continue;
            if (EmitCNeedsFullSyms::check(nodep)) return true;
        }
        return false;
    }
    void openNextOutputFile(const std::string& fileName, bool needsFullSyms) {
        ...
        // The precompiled header holds only design-independent declarations, so pull in the
        // headers this file actually needs. Files that do not reference another scope make do
        // without the complete symbol table, and so do not depend on every module's header.
        if (needsFullSyms) {
            puts("#include \"" + EmitCUtil::symClassName() + ".h\"\n");
        } else {
            puts("#include \"" + EmitCUtil::prefixNameProtect(m_fileModp) + ".h\"\n");
        }
    ...
    void emitCtorImp(const AstNodeModule* modp) {
        ...
        if (modp->isTop()) {
            ...
            puts("vlNamep = strdup(Verilated::catName(vlSymsp->__Vm_namep, namep));\n");
        }
    ...
    void emitCommonImp(const AstNodeModule* modp) {
        ...
        if (hasCommonImp(modp) || hasCommonImp(classp)) {
            openNextOutputFile(m_fileBaseName,
                               commonImpNeedsFullSyms(modp) || commonImpNeedsFullSyms(classp));
    ...
    void emitCFuncImp(const AstNodeModule* modp) {
        ...
        // Open output file. Conservatively use the union over all functions of this module,
        // as --output-split may divide them over several files at points not known yet.
        m_needsFullSyms = EmitCNeedsFullSyms::check(funcps);
        openNextOutputFile(m_uniqueNames.get(m_fileBaseName), m_needsFullSyms);
        // Emit all functions
